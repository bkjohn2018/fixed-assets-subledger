-- BIP extract: AP aging schedule fact (F_AP_Aging_Schedule)
-- CSV: ap_aging_schedule_{yyyymmdd}.csv
-- Parameter: @AS_OF_DATE (default TRUNC(SYSDATE) in BIP data model)

WITH params AS (
    SELECT TRUNC(CAST(@AS_OF_DATE AS DATE)) AS as_of_date
    FROM dual
),
